...
export default function MultiGeotagField () {
    var _this = this
...
MultiGeotagField.prototype.bindSingleField = function (element) {
    ...
    if ($input.val() !== '') {
        try {
            ...
            for (var i = 0; i < geocodeslength; i++) {
                ...
                window.google.maps.event.addListener(markers[i], 'dragend', $.proxy(_this.setMarkerEvent, _this, markers[i], markers, $input, $geocodeReset, map))
            }
            $geocodeReset.show()
        } catch (e) {
            $input.show()
    ...
    window.google.maps.event.addListener(map, 'click', $.proxy(_this.setMarkerEvent, _this, null, markers, $input, $geocodeReset, map))
    window.google.maps.event.addListener(map, 'click', $.proxy(_this.syncSelector, _this, $selector, markers, map, $input))
    ...
    window.Rozier.$window.on('resize', $.proxy(_this.resetMap, _this, map, markers, mapOptions))
