 ...
 * Markdown Editor
 */
export default function MarkdownEditor ($textarea, index) {
    var _this = this

    _this.markdownit = new Markdownit()
    _this.markdownit.use(markdownItFootnote)
...
MarkdownEditor.prototype.init = function () {
    ...
    if (_this.$cont.length &&
       _this.$textarea.length) {
        ...
        window.Rozier.$window.on('keyup', $.proxy(_this.echapKey, _this))
...
MarkdownEditor.prototype.closePreview = function (e) {
    ...
    TweenLite.fromTo(_this.$preview, 1, {x: 0, opacity: 1}, {x: width * -1,
        opacity: 0,
        ease: Expo.easeOut,
        onComplete: function () {
